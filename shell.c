 ...
 */
static int command( int input, bool isFirstCommand, bool isLastCommand )
{
	int pipettes[2];
	...
	*/
	...
	if (pid == 0) {
		if( isFirstCommand == true && isLastCommand == false && input == 0 ) {
			// First command
			dup2( pipettes[WRITE], STDOUT_FILENO );
		} else if( isFirstCommand == false && isLastCommand == false && input != 0 ) {
			// Middle command
	...
	if( isLastCommand == true ) close( pipettes[READ] );
